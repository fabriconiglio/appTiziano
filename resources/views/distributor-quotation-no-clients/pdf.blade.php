...
<body>
    ...
    <table class="header-table">
        <tr>
            <td class="header-left">
                ...
                <div class="doc-info"><strong>A:</strong> {{ $quotation->nombre ?? 'Cliente no especificado' }}</div>
                <div class="doc-info"><strong>Teléfono:</strong> {{ $quotation->telefono ?? '-' }}</div>
    ...
    <table class="info-table">
        <tr>
            <td style="width: 50%;">
                <span class="info-label">Email:</span> {{ $quotation->email ?? '-' }}<br>
                <span class="info-label">Dirección:</span> {{ $quotation->direccion ?? '-' }}
            </td>
            <td style="width: 50%;">
                <span class="info-label">Válido hasta:</span> {{ $quotation->valid_until->format('d/m/Y') }}<br>
                <span class="info-label">IVA:</span> {{ $quotation->tax_percentage }}% &nbsp;&nbsp; <span class="info-label">Descuento:</span> {{ $quotation->discount_percentage ?? 0 }}%<br>
                <span class="info-label">Pago:</span> {{ $quotation->payment_terms ?? '-' }} &nbsp;&nbsp; <span class="info-label">Entrega:</span> {{ $quotation->delivery_terms ?? '-' }}
            </td>
        </tr>
    </table>
