...
@section('title', 'Editar Presupuesto - ' . $distributorClient->full_name)
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 text-dark">Editar Presupuesto - {{ $distributorClient->full_name }}</h5>
                        <a href="{{ route('distributor-clients.quotations.show', [$distributorClient, $quotation]) }}" class="btn btn-secondary btn-sm">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('distributor-clients.quotations.update', [$distributorClient, $quotation]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-4">
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control @error('quotation_number') is-invalid @enderror"
                                           ...
                                           value="{{ old('quotation_number', $quotation->quotation_number) }}" required readonly>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('quotation_date') is-invalid @enderror"
                                           ...
                                           value="{{ old('quotation_date', $quotation->quotation_date->format('Y-m-d\TH:i')) }}" required>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="datetime-local" class="form-control @error('valid_until') is-invalid @enderror"
                                           ...
                                           value="{{ old('valid_until', $quotation->valid_until->format('Y-m-d\TH:i')) }}" required>
                                    @error('valid_until')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select @error('quotation_type') is-invalid @enderror" 
                                        ...
                                        <option value="al_por_mayor" {{ old('quotation_type', $quotation->quotation_type) === 'al_por_mayor' ? 'selected' : '' }}>Al Por Mayor</option>
                                        <option value="al_por_menor" {{ old('quotation_type', $quotation->quotation_type) === 'al_por_menor' ? 'selected' : '' }}>Al Por Menor</option>
                                    </select>
                            ...
                            <div class="row mb-4">
                                <div class="col-md-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control @error('tax_percentage') is-invalid @enderror"
                                               ...
                                               value="{{ old('tax_percentage', $quotation->tax_percentage) }}" min="0" max="100" step="0.01" required>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control @error('discount_percentage') is-invalid @enderror"
                                               ...
                                               value="{{ old('discount_percentage', $quotation->discount_percentage) }}" min="0" max="100" step="0.01">
                                        <span class="input-group-text">%</span>
                                    </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select @error('payment_terms') is-invalid @enderror" 
                                        ...
                                        <option value="efectivo" {{ old('payment_terms', $quotation->payment_terms) == 'efectivo' ? 'selected' : '' }}>Efectivo</option>
                                        <option value="tarjeta" {{ old('payment_terms', $quotation->payment_terms) == 'tarjeta' ? 'selected' : '' }}>Tarjeta</option>
                                        <option value="transferencia" {{ old('payment_terms', $quotation->payment_terms) == 'transferencia' ? 'selected' : '' }}>Transferencia</option>
                                        <option value="cheque" {{ old('payment_terms', $quotation->payment_terms) == 'cheque' ? 'selected' : '' }}>Cheque</option>
                                    </select>
                                    @error('payment_terms')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control @error('delivery_terms') is-invalid @enderror"
                                           ...
                                           value="{{ old('delivery_terms', $quotation->delivery_terms) }}" placeholder="Ej: Inmediata, 48hs">
                            ...
                            <div class="mb-3">
                                <label class="form-label">
                                    Productos del Presupuesto 
                                    <span class="badge bg-primary" id="product-counter">{{ count($quotation->products_quoted) }}</span>
                                </label>
                                <div id="products-container">
                                    @if(count($quotation->products_quoted) > 0)
                            ...
                            <div class="summary-section">
                                ...
                                <div class="summary-row">
                                    <span>Subtotal:</span>
                                    <span id="subtotal-display">${{ number_format($quotation->subtotal, 2) }}</span>
                                </div>
                                <div class="summary-row">
                                    <span>IVA ({{ $quotation->tax_percentage }}%):</span>
                                    <span id="tax-display">${{ number_format($quotation->tax_amount, 2) }}</span>
                                </div>
                                <div class="summary-row">
                                    <span>Total con IVA:</span>
                                    <span id="total-with-tax-display">${{ number_format($quotation->total_amount, 2) }}</span>
                                </div>
                                <div class="summary-row">
                                    <span>Descuento ({{ $quotation->discount_percentage }}%):</span>
                                    <span id="discount-display">${{ number_format($quotation->discount_amount, 2) }}</span>
                                </div>
                                <div class="summary-row summary-total">
                                    <span>Total Final:</span>
                                    <span id="final-total-display">${{ number_format($quotation->final_amount, 2) }}</span>
                                </div>
                            </div>
                            ...
                            <div class="row mb-4 mt-5">
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('observations') is-invalid @enderror" id="observations" name="observations" rows="3" placeholder="Observaciones adicionales">{{ old('observations', $quotation->observations) }}</textarea>
                                    @error('observations')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('terms_conditions') is-invalid @enderror" id="terms_conditions" name="terms_conditions" rows="3" placeholder="Términos y condiciones del presupuesto">{{ old('terms_conditions', $quotation->terms_conditions) }}</textarea>
                            ...
                            <div class="row">
                                <div class="col-md-12 text-center">
                                    ...
                                    <a href="{{ route('distributor-clients.quotations.show', [$distributorClient, $quotation]) }}" class="btn btn-secondary btn-lg">
